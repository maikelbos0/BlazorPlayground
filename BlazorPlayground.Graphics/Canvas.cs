...
public class Canvas {
    ...
    public const int DefaultZoom = 100;
    ...
    public int Zoom {
        get => zoom;
        set => zoom = Math.Max(value, MinimumZoom);
    }

    public int DisplayWidth => Width * Zoom / 100;
    
    public int DisplayHeight => Height * Zoom / 100;
    ...
    public void StartActionExecution(MouseEventArgs mouseEventArgs) {
        StartPoint = GetPoint(mouseEventArgs);
        IsSecondaryAction = mouseEventArgs.CtrlKey;
    }

    public void UpdateActionExecution(MouseEventArgs mouseEventArgs) {
        if (StartPoint != null) {
            EndPoint = GetPoint(mouseEventArgs);
        }
    }

    private Point GetPoint(MouseEventArgs mouseEventArgs)
